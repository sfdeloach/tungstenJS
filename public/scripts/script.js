...
function editParticipant(row) {
  ...
  worksheetShowEditRow =
    '<tr>' +
    '<td>' +
    "<input class='form-control' list='participants' name='assessment[dept_id]' type='text' disabled value='" +
    row.deptID +
    "'>" +
    '</td>' +
    "<td class='col-width12'><input class='form-control' name='assessment[eval_date]' type='date' required value='" +
    date +
    "'></td>" +
    "<td><input class='form-control' name='assessment[weight]' type='number' step=1 min=0 required value='" +
    row.weight +
    "'></td>" +
    "<td><input class='form-control' name='assessment[heart_rate]' type='text' autocomplete='off' required value='" +
    row.hr +
    "'></td>" +
    "<td><input class='form-control' name='assessment[blood_pressure]' type='text' autocomplete='off' required value='" +
    row.pressure +
    "'></td>" +
    "<td><input class='form-control' name='assessment[body_fat]' type='number' step=0.1 min=0 required value='" +
    row.body +
    "'></td>" +
    "<td><input class='form-control' name='assessment[flex]' type='number' step=0.1 min=0 required value='" +
    row.flex +
    "'></td>" +
    "<td><input class='form-control' name='assessment[situp]' type='number' step=1 min=0 required value='" +
    row.situp +
    "'></td>" +
    "<td><input class='form-control' name='assessment[bench]' type='number' step=1 min=0 required value='" +
    row.bench +
    "'></td>" +
    "<td><input class='form-control' name='assessment[leg]' type='number' step=1 min=0 required value='" +
    row.leg +
    "'></td>" +
    '<td>' +
    ...
    '</select>' +
    '</td>' +
    "<td class='col-width03'><input class='form-control' name='assessment[cardio_min]' type='number' step=1 min=0 required value='" +
    min +
    "'></td>" +
    "<td class='col-width03'><input class='form-control' name='assessment[cardio_sec]' type='number' step=1 min=0 required value='" +
    sec +
    "'></td>" +
    "<td><input class='form-control' name='assessment[cardio_heartrate]' type='number' step=1 min=0 value='" +
    row.cardioHR +
    "'></td>" +
    '<td>' +
...
function showParticipant(row) {
  ...
  var worksheetShowUpdateRow =
    ...
    '</td>' +
    '<td>' +
    new Date(row.eval_date).toLocaleDateString('en-US') +
    '</td>' +
    '<td>' +
    row.weight +
    '</td>' +
    ...
    '</button>' +
    '<form class="delete-form" action="/worksheets/' +
    row.worksheet_id +
    '/' +
    row.assessment_id +
...
$(document).ready(function() {
    ...
    if ($('#partnew-id').prop('disabled')) {
      ...
      $.ajax({
        ...
        .done(function(ids) {
          ...
          if (ids === 'error') {
            alert(
              'Sorry, there was a problem connecting to the database, please check the connection.'
            );
  ...
  $('#showTableBody').on('click', '.workshow-edit', function(eventObject) {
    ...
    $(this)
      .parent()
      .parent()
      .replaceWith(editParticipant(row));
    ...
    $.ajax({
      ...
      .done(function(row) {
        ...
        that
          .parent()
          .parent()
          .replaceWith(showParticipant(row));
        // enable edit buttons
  ...
  $('#newassessment-form').on('submit', function(eventObject) {
    ...
    $.ajax({
      url: '/ajax/participants/id_exists',
      data: dept_id,
      type: 'POST',
      async: false,
    })
      .done(function(isValid) {
        ...
        if (isValid) {
          ...
          alert(
            'You must provide a valid ID from the list. If you cannot find the name you are looking for, make sure you have created one in the Participant section.'
          );
        }
      })
      .fail(function(xhr, status, errorThrown) {
