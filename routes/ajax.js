...
router.get('/participants/get_ids', authorization.isEditor, function(req, res) {
  var key,
    result = [];
  Participant.find({ group: 'pd' }, { dept_id: 1, _id: 0 }, function(
    err,
    foundParticipants
  ) {
    if (err) {
...
router.put('/worksheets/:worksheet_id', authorization.isEditor, function(
  ...
  updateData.eval_date = dateHelper.htmlToDb(
    updateData.eval_date + 'T00:00:00.000Z'
  );
  updateData.cardio.time =
    updateData.cardio.min +
    ':' +
    (parseInt(updateData.cardio.sec, 10) < 10
      ? '0' + parseInt(updateData.cardio.sec, 10)
      : parseInt(updateData.cardio.sec, 10));
  updateData.cardio.heart_rate =
    updateData.cardio.type === 'walk' && updateData.cardio.heart_rate.length > 0
      ? parseInt(updateData.cardio.heart_rate, 10)
      : null;
